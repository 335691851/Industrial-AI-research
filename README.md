...
全量和增量使用同一套自主研究 Harness。全量重研覆盖全部默认方向和用户配置，检索最近一年，完成后归档旧快照并重建当前展示；增量研究每天 19:00 自动检索上次成功运行后的变化（向前重叠 48 小时且新闻窗口不超过 5 天），再与历史数据融合。Tavily 统一使用 Search Advanced，不使用 Research API。系统按覆盖率补搜，每次最多让 100 份有效来源进入分析。
