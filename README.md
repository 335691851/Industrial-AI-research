...
> **2026-09-14｜每日增量搜索降耗**
>
> 增量使用独立的轻量搜索计划：每家重点企业每日一次综合查询，保留全部关键词、研究方向与政策/技术/龙头扫描，首轮 Basic，自主问题最多 2 个；Advanced 补搜最多 3 个问题。默认配置首轮 34 credits，单次 Search 预算 34–42 credits；配置数量增加会相应增加首轮费用，Extract 和失败节点恢复重发另计。全量重研保留原搜索计划。增量官网画像仅在缺失、不完整、官网变更或超过 7 天时刷新，近期有效画像复用。运行记录新增“增量预算”。部署后由下一次新启动的增量任务生效，实际成本与覆盖效果需运行后核验。

> [!NOTE]
> **近期业务与系统更新（2026-09-11）**
>
> - **发布快照统一**：前台展示最近一次成功研究的完整发布快照，不再在页面读取或跨天时自行删除数据。全量重研或每日 19:00 增量研究成功到达“融合发布”节点后，才统一增补新事件、剔除超过 30 天的事件、刷新企业画像与综合洞察，因此统计卡片、情报列表和运行结果在两次发布之间保持一致。
> - **研究流程增强**：全量与增量共用同一套 LangGraph Harness，通过自主规划、常规行业扫描、配置关键词扩展、重点企业追踪、指定网站采集、覆盖检查及补充搜索完成研究；Tavily 对常规任务使用 Basic，对技术前沿、政府政策、资本、官网与补搜使用 Advanced，Search 同步返回清洗后的可信原文以降低源站二次抓取失败。每轮最多 100 份事件正文进入分析，企业官网画像材料使用独立容量。
> - **来源质量增强**：来源分为一手、权威媒体、工业专业媒体和不可信来源；小媒体与聚合转载剔除，权威媒体和交叉印证补足官网之外的重要事件。事件按企业别名、标题／摘要相似度、原文引用、标准化 URL 和时间范围跨批次去重；无法核验的日期使用首次收录日作为有效期依据，但不冒充原文发布日期。
> - **企业与主题覆盖增强**：重点企业逐家执行产品/方案、资本/战略、官网动态三类检索，默认补入创新奇智；物理 AI 衍生技术、工业智能新能力、国家/部委动作与全球产业政策使用独立检索任务。发布组合限制单一企业占比并先保留已取得的重点企业和类别。
> - **组合数量与占比再平衡**：正文读取配额约 40% 用于行业、政府、前沿技术与龙头公司，重点企业约 30%；政府、前沿、资本及国内外龙头合作不足 3 份正文时自动 Advanced 补搜。单个重点企业最多展示 3 条、其他企业最多 2 条，目标为 45–54 条有效情报；证据不足不会为凑数生成内容。
> - **数量与证据误损修复**：全量重研会将本轮发现与最近 30 天仍有效的历史核验事件融合并重新过门，避免搜索波动提前删除历史；关键数字在完整引用正文中核验，不再因为模型选择的短摘录未包含数字而误删。优先关注最多展示 3 条，官方／政府／监管等可问责来源可单独支撑高置信度重要事件。
> - **企业官网研究增强**：配置的重点企业始终保留并优先刷新，企业叙事、定位、产品方案与技术能力主要使用官网长期档案，不受新闻 30 天窗口限制。公开 JavaScript 动态页面可复用 `TAVILY_API_KEY` 进行 Extract Advanced 正文兜底；`#/…` 路由会保留，同域首页只有在明确包含目标企业身份时才能作为官网首页资料接纳，并记录实际证据 URL。
> - **展示体验更新**：页面采用白底科技主题和紧凑页头，左侧导航支持抽屉收起并与已保存研究关键词同步；优先关注支持多条轮播；综合洞察默认限制在半屏内，完整显示结论标题，内容较多时可滚动或展开查看全部推理、业务启示与证据。
> - **模型配置校验**：模型名称改为受控清单并在服务端再次校验，避免因模型标识拼写或供应商不支持而导致研究任务失败。
...
- 新闻、企业动态、投融资资讯在每次成功发布时统一维护最近 30 天有效窗口；前台在下一次成功发布前保持快照不变，企业官网画像长期保留。

## 研究 Harness

```mermaid
flowchart LR
  P[Agent 自主规划] --> C[Basic + Advanced 搜索与采集]
  C --> G[覆盖检查]
  G -->|存在缺口| S[定向补充搜索]
  G -->|覆盖充分| E[结构化解析]
  S --> E
...
全量和增量使用同一套自主研究 Harness。全量覆盖检索最近 30 天，并将新发现与窗口内仍有效的历史核验事件融合后重新校验；增量每天 19:00 自动检索上次成功运行后的变化并融合有效历史。企业官网画像不受新闻窗口限制。证据校验后，使用完整有效事件集合生成综合洞察，随新闻一起发布；洞察失败不阻断新闻发布。Tavily 使用 Search Basic/Advanced 混合策略，每次最多 100 份事件正文进入分析，企业画像材料不占事件容量。产品与技术细则以 `说明.md` 为准。
...
| `TAVILY_API_KEY`            | 启用 Search Basic/Advanced、全网研究规划和覆盖补搜                   |
...
采集只接受公网 HTTP(S)，限制端口、重定向、DNS 解析及连接目标，阻止 loopback、内网、链路本地和 DNS 重绑定。服务端不直接执行网页脚本；公开动态页面仅在普通 HTML 无有效正文时调用受限的 Extract Advanced 兜底。页面中的指令作为不可信数据处理，模型只能输出结构化研究结果。
...
- HTML / RSS 采集已实现；公开动态页面可尝试高级正文提取，但需要登录、验证码或人工验证的站点不会绕过限制，PDF 全文和付费数据库仍属于当前采集器边界。
