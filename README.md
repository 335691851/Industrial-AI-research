...
> - **研究流程增强**：全量与增量共用同一套 LangGraph Harness，通过自主规划、常规行业扫描、配置关键词扩展、重点企业追踪、指定网站采集、覆盖检查及补充搜索完成研究；Tavily Search 使用 `advanced`，异步分批处理，每轮最多 100 份有效来源进入分析。
> - **来源质量增强**：事件按企业别名、标题／摘要相似度、原文引用、标准化 URL 和时间范围跨批次去重；重大信号按重要度、可信度与多源证据共同判定；无法核验的日期使用首次收录日作为有效期依据，但不冒充原文发布日期。
