...
打开 http://127.0.0.1:3000 。不配置云服务也能使用本地持久化工作区，默认显示明确标注的演示内容。它们不是最新新闻，也不应用作研究结论。
...
- 情报展示：重大信号、主题趋势、企业与方向热力图、情报搜索 / 分类 / 重要度排序、企业画像详情与来源原文。
- 企业画像：叙事、定位、产品方案、能力、投融资披露、对炽橙的分析启示。
- 来源配置：关键词、研究企业、网站 / RSS / 微信公众号公开链接，支持增删、编辑、启停与保存。
- 智能体记录：DeepSeek、OpenAI、GLM、Qwen 密钥与模型名配置，最终选择保存；运行状态、节点日志、手动全量 / 增量研究及失败恢复。
...
代码在 `src/server/research.ts`：真实 `StateGraph`，而非模拟的前端运行状态。Harness 包含源站并发预算、页面大小与超时限制、模型限流重试、结构化输出校验、引用原文比对、幂等发布、每日幂等 ID、工作区互斥与失败恢复。
...
全量重研提高每个站点的采集深度和单轮材料预算；增量研究采用较小预算，两者都合并历史而非删除记录。全量不等于无限制抓取整个互联网。每次最多分析 24 / 32 份材料，按来源轮转分配，超出预算会写入日志。
...
| `TAVILY_API_KEY`            | 可选，增加跨站关键词 / 企业新闻发现                                  |

生产环境不会降级到临时文件存储。完全未配置 Supabase 时提供明确标注的只读演示预览；配置不完整或连接失败会报错。缺少 `DATABASE_URL` 会阻止研究启动。
...
每日 ID 保证两个调度器重复触发也不会重复发布。服务器失败后可从页面恢复，或用同一天的 Cron 重试；平台本身不自动重试 Vercel Cron。过期运行会在下一次启动时标记失败并释放互斥。
