...
> - **研究流程增强**：全量与增量共用同一套 LangGraph Harness，通过自主规划、常规行业扫描、配置关键词扩展、重点企业追踪、指定网站采集、覆盖检查及补充搜索完成研究；Tavily 对常规任务使用 Basic，对技术前沿、政府政策、资本、官网与补搜使用 Advanced，异步分批处理，每轮最多 100 份有效来源进入分析。
...
全量和增量使用同一套自主研究 Harness。全量检索最近 30 天并重建新闻展示；增量每天 19:00 自动检索上次成功运行后的变化并融合有效历史。企业官网画像不受新闻窗口限制。证据校验后，使用完整有效事件集合生成综合洞察，随新闻一起发布；洞察失败不阻断新闻发布。Tavily 使用 Search Basic/Advanced 混合策略，每次最多 100 份有效来源进入分析。产品与技术细则以 `说明.md` 为准。
